...
class GcnEncoderGraph(nn.Module):
    ...
    def build_pred_layers(
        ...
        if len(pred_hidden_dims) == 0:
            ...
            # #to be deleted
            # pred_layers.append(nn.Linear(pred_dim, pred_dim))
            # pred_layers.append(nn.ReLU())
            # #####

            pred_layers.append(nn.Linear(pred_dim, label_dim))
            pred_model = nn.Sequential(*pred_layers).to(self.device)
        return pred_model
    ...
    def gcn_forward(
        ...
        adj_att_tensor = None #torch.stack(adj_att_all, dim=3)
    ...
    def forward(self, x, adj, batch_num_nodes=None, adj_v=None, extract_feats=False, node_mask=None, attn=False, **kwargs):
        x = x.to(self.device)
        adj = adj.to(self.device)
        # mask
        ...
        # sparse_mask = torch.count_nonzero(x, dim=2)
        # sparse_mask = sparse_mask > 0

        # sparse_adj_mask = torch.count_nonzero(adj, dim=1)
        # sparse_adj_mask = sparse_adj_mask > 0
        ...
        if adj_v is not None:
            ...
            diag_mask = (1. - torch.eye(adj_mask.shape[1]).float()).unsqueeze(0)
        ...
        # self.embedding_mask = self.embedding_mask * sparse_mask.unsqueeze(2)
        # self.embedding_mask = self.embedding_mask * sparse_adj_mask.unsqueeze(-1)

        if node_mask is not None:
            self.embedding_mask = self.embedding_mask * node_mask.reshape((1, node_mask.shape[0], 1))
    ...
    def getEmbeddings(self, x, adj, batch_num_nodes=None, adj_v=None, extract_feats=False, **kwargs):
        x = x.to(self.device)
        adj = adj.to(self.device)
        # mask
        max_num_nodes = adj.size()[1]
...
class GcnEncoderNode(GcnEncoderGraph):
    ...
    def forward(self, x, adj, batch_num_nodes=None, new_node_idx = None, **kwargs):
        # mask
        ...
        # print("embed: ", x.shape, self.embedding_tensor.shape)
