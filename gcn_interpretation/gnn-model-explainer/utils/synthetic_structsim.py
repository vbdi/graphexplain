...
def cycle(start, len_cycle, role_start=0):
    ...
    # return grid(start, dim=3, role_start=role_start)
    graph = nx.Graph()
    ...
    #mb
    # graph.add_edges_from([(start + 2, start)])
    # # graph.add_edges_from([(start + 3, start)])
    # graph.add_edges_from([(start + 3, start+1)])
    # # graph.add_edges_from([(start + 4, start+1)])
    # graph.add_edges_from([(start + 4, start+2)])
    # # graph.add_edges_from([(start + 5, start+2)])
    # graph.add_edges_from([(start + 5, start+3)])
    #
    # graph.add_edges_from([(start, start+4)])
    # graph.add_edges_from([(start+1, start+5)])



    # print("start: ", start, role_start)
...
def house_mod(start, role_start=0):
...
def grid(start, dim=2, role_start=0):
    ...
    # print(start, dim, role_start)

    # for e in grid_G.edges():
    #     print(e)
...
'''
cycleAA: 5-cycle into 5-cycle
cycleAB: 4-cycle into 5-cycle
cycleBB" 4-cycle into 4-cycle
'''

def cycleAA(start, role_start=0):
    """Builds a cycle-like  graph (style AA), with index of nodes starting at start
    and role_ids at role_start
    INPUT:
    -------------
    start       :    starting index for the shape
    ...
    OUTPUT:
    -------------
    graph       :    a cycke shape graph, with ids beginning at start
    roles       :    list of the roles of the nodes (indexed starting at
                     role_start)
    """
    graph = nx.Graph()
    ...
    # graph.add_edges_from([(start, start + 2), (start + 1, start + 3)])
    roles = [role_start, role_start, role_start, role_start, role_start, role_start, role_start, role_start, role_start]
    return graph, roles

def cycleAB(start, role_start=0):
    """Builds a cycle-like  graph (style AB), with index of nodes starting at start
    and role_ids at role_start
    INPUT:
    -------------
    start       :    starting index for the shape
    ...
    # graph.add_edges_from([(start, start + 2), (start + 1, start + 3)])
    roles = [role_start, role_start, role_start, role_start, role_start, role_start, role_start, role_start]
    return graph, roles

def cycleBB(start, role_start=0):
    """Builds a cycle-like  graph (style BB), with index of nodes starting at start
    and role_ids at role_start
    INPUT:
    -------------
    start       :    starting index for the shape
    ...
    """
    graph = nx.Graph()
    graph.add_nodes_from(range(start, start + 7))
    graph.add_edges_from(
        [
            (start, start + 1),
            (start + 1, start + 2),
...
def dualPair(start, role_start=0):
    """Builds a dual cycle-like  graph, with index of nodes starting at start
    and role_ids at role_start
    INPUT:
    -------------
    start       :    starting index for the shape
    ...
    """
    graph = nx.Graph()
    graph.add_nodes_from(range(start, start + 2))
    graph.add_edges_from(
        [
            (start, start + 1),
        ]
    ...
    # graph.add_edges_from([(start, start + 2), (start + 1, start + 3)])
    roles = [role_start, role_start]
    return graph, roles

def triplet(start, role_start=0):
    """Builds a triple  graph, with index of nodes starting at start
    and role_ids at role_start
    INPUT:
    -------------
    start       :    starting index for the shape
    ...
    # graph.add_edges_from([(start, start + 2), (start + 1, start + 3)])
    roles = [role_start, role_start + 1, role_start + 1]
    return graph, roles
...
def build_graph(
    ...
    for shape_id, shape in enumerate(list_shapes):
        ...
        try:
            ...
            if shape_type == "cycleAA" or shape_type == "cycleBB":
                col_start = 1
            elif shape_type == "cycleAB":
                col_start = 2
            else:
